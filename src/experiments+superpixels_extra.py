...
def print_image(clustering_model, data, x, y, num_nodes_view, all_concepts, path, task='local', concepts=None):
    ...
    for index, i in enumerate(l):
        ...
        if c_tmp not in used_concepts:
            ...
            for j in range(1, cols+1):
                ...
                counter += 1
    fig.tight_layout()
    plt.savefig(os.path.join(path, f"{task}_img_concepts.svg"))
    ...
    for index, el in enumerate(to_print):
        ...
        plt.axis("off")
        plt.savefig(f"./images/mnist_{index}_image.png")
        im = matplotlib.image.imread(f"./images/mnist_{index}_image.png")
        top_plot += [im]
    ...
    for i, ax_list in zip(l, axes):
        ...
        for ax, new_G, color_map, g_label, g_concept, pos, g_raw in zip(ax_list, top_graphs, color_maps, labels, concepts_list, node_pos, raw_concepts):
            if counter == 0:
                ...
                nx.draw(new_G, node_color=color_map, pos=pos, ax=fig_aux.add_subplot(), node_size=100)
                plt.axis('off')
                fig_aux.savefig(f"./images/mnist_{i}_graph.png")
                im = matplotlib.image.imread(f"./images/mnist_{i}_graph.png")
                top_plot += [im]
...
def main():
    ...
    train_loader, train, test_loader, test, weights = data_utils.create_mnist_superpixels(SIZE, BATCH_SIZE, NUM_CLASSES)
    ...
    # model training
    if MODE == 'vanilla':
        model = models.MNISTSuperpixels_Vanilla(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = False
    elif MODE == 'single_vanilla' or MODE == 'anchors':
        model = models.MNISTSuperpixels_SingleVanilla(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = False
    elif MODE == 'single_CBM':
        model = models.MNISTSuperpixels_SingleCBM(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = True
    elif MODE == 'multi_CBM':
        model = models.MNISTSuperpixels_MultiCBM(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = True
    elif MODE == 'sharcs_no_batch':
        model = models.MNISTSuperpixels_no_batch(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = True
        MODE = 'sharcs'
    else:
        model = models.MNISTSuperpixels_missing(CLUSTER_ENCODING_SIZE, LOCAL_NUM_CLASSES, NUM_CLASSES)
        interpretable = True
    model.to(dev)
    ...
    # train
    train_acc, test_acc, train_loss, test_loss = train_graph_class(model, train_loader, test_loader, weights, EPOCHS, LR,
                                                                   NUM_CLASSES, if_interpretable_model=interpretable,
                                                                   mode=MODE)
    ...
    if interpretable:
        ...
        top_plot, top_concepts = plot_samples(None, g_concepts, expanded_x, expanded_pos, expanded_batch, y_g, len(centroids), 5, edges_t, concepts_g_local, path, task='local', concepts=centroids)
        ...
        # plot clustering
        print(concepts_img_local, centroids)
        ...
        if MODE != 'single_CBM':
            ...
            g_concepts = graph_concepts.detach().cpu().numpy()
            print('GRAPH CONCEPTS')
            top_plot, top_concepts = plot_samples(None, g_concepts, expanded_x, expanded_pos, expanded_batch, y, len(centroids), 5, edges_t, concepts, path, task='global', concepts=centroids)
            ...
            test_img = test_data.img
            x_tab = torch.cat((train_img, test_img))
            top_plot_images, top_concepts_img = print_image(None, t_concepts, x_tab, y, 5, concepts, path, task='global', concepts=centroids)
            ...
            wandb.log({'missing graph modality accuracy': missing_accuracy[0], 'missing image modality accuracy': missing_accuracy[1]})
